...
<tr>
	<td>
		<c:forEach var="tp" items="${ tpList }">
			<a class="btn btn-sm p-0" href="../detail/detail.do?type=1&no=${ tp.no }">${ tp.name }</a>
		</c:forEach>
		<c:choose>
...
<tr>
	<td>
		<c:forEach var="rs" items="${ rsList }">
			<a class="btn btn-sm p-0" href="../detail/detail.do?type=2&no=${ rs.no }">${ rs.name }</a>&nbsp;
		</c:forEach>
		<c:choose>
...
<tr>
	<td>
		<c:forEach var="fs" items="${ fsList }">
			<a class="btn btn-sm p-0" href="../detail/detail.do?type=3&no=${ fs.no }">${ fs.name }</a>&nbsp;
		</c:forEach>
		<c:choose>
...
<tr>
	<td>
		<c:forEach var="ht" items="${ htList }">
			<a class="btn btn-sm p-0" href="../detail/detail.do?type=${ ht.type }&no=${ ht.no }">${ ht.name }</a>&nbsp;
		</c:forEach>
		<c:choose>
